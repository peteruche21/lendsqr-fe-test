...
export function UsersPage() {
    ...
    const totalPages = useMemo(() => Math.max(1, Math.ceil(tableTotal / pageSize)), [tableTotal, pageSize]);
    const stats = useMemo(
        () =>
            createUserStats({
                total: statsTotal,
        ...
        () =>
            createUserColumns({
                ...
                onOpenFilter: (field, trigger) => {
                    ...
                    const preferredLeft = field === 'status' ? rect.right - FILTER_POPOVER_WIDTH : rect.left;
                    const maxLeft = window.innerWidth - FILTER_POPOVER_WIDTH - FILTER_POPOVER_VIEWPORT_MARGIN;
    ...
    const handleFilterChange = <Field extends UserFilterField>(field: Field, value: UserFilters[Field]) => {
        setCurrentPage(1);
        setFilters((current) => ({
            ...current,
            [field]: value,
            ...
            <section className="users">
                ...
                <footer className="users__footer">
                    <div className="users__footer-left">
                        ...
                        <PageSizeDropdown onChange={handlePageSizeChange} value={pageSize} />
                        <span className="users__footer-text">out of {tableTotal}</span>
                        {statusMessage && <span className="users__status">{statusMessage}</span>}
                    </div>
...
function createUserStats({ total, users }: { total: number; users: User[] }): UserStatCard[] {
...
function UserStatCard({ stat }: { stat: UserStatCard }) {
    return (
        <Card as="article" className="users-stat-card">
            <div className="users-stat-card__icon" style={{ backgroundColor: stat.backgroundColor }}>
                {stat.icon}
            </div>
