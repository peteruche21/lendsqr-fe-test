...
export function UserDetailPage() {
    ...
    const userWithStatus = user ? { ...user, status: getUserStatus(user.id, user.status) } : null;

    const navigation = (
        <button className="user-detail-page__back" type="button" onClick={() => navigate('/users')}>
            <MoveLeft size={30} />
    ...
    return (
        <Page header={<PageHeader actions={actions} navigation={navigation} title="User Details" />} showLogo={false}>
            <section className="user-detail-page">
                <UserDetailsSummary user={userWithStatus} activeTab={activeTab} onTabChange={setActiveTab} />

                <div className="user-detail-page__tab-content">
                    {activeTab === 'general' && (
                        <div className="user-detail-page__placeholder">General Information displays here.</div>
                    )}
                    {activeTab === 'documents' && (
                    ...
                    {activeTab === 'loans' && <div className="user-detail-page__placeholder">Loans display here.</div>}
                    {activeTab === 'savings' && (
